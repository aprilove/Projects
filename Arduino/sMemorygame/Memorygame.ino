...
void setup() {
  ...
  for (int thisPin = 0; thisPin < 4; thisPin++) {
    pinMode(ledPins[thisPin+8], OUTPUT);
  }
    for (int thisbPin = 0; thisbPin < 4; thisbPin++) {
    pinMode(buttonPins[thisbPin+4], INPUT);
  }
  pinMode(buzzer, OUTPUT);
}

void loop() {
  Serial.print("Press any button to start");
...
void turn() {
  ...
  runNum = 0;
  while(runNum < 4){
    list.add(runNum,random(4)+8);
    digitalWrite(list.get(runNum),HIGH);
    delay(timer);
    digitalWrite(list.get(runNum),LOW);
    runNum++;
    timer -= 10;
    Serial.print(runNum);
  }
  Serial.print("turnend");
}
...
bool check(){
  ...
  for (int i = 0; i < runNum+1; i++){
    do {
      ...
      btimer++;
      delay(10);
    } while(digitalRead(list.get(i)-4 != LOW));
    if (btimer > 1000){
      tone(buzzer, 1000);
      delay(1000);
      noTone(buzzer);
